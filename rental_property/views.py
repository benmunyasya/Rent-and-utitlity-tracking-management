...
def managers_dashboard(request):
    manager1=Managers.objects.get(associated_account=request.user)
    buildings= Building.objects.filter(manager=manager1)
    buildings_count = Building.objects.filter(manager=manager1).count()
    rentalunits=RentalUnit.objects.all()
    rentalunits_count=0
    managed_rental_units=[]
    for i in rentalunits:
        if i.building  in buildings:
            rentalunits_count=rentalunits_count+1
            managed_rental_units.append(i)
    tenants_count=0
    tenants=Tenants.objects.all()
    for i  in tenants:
        if i.rented_unit in managed_rental_units:
            tenants_count=tenants_count+1
    occupied_units_count=0
    for i in managed_rental_units:
        if i.status=='occupied':
            occupied_units_count=occupied_units_count+1
    ...
    context = {
       ...
       'tenants_count':tenants_count,
        'rentalunits_count':rentalunits_count,
       'occupied_units_count': occupied_units_count,
       'user_loggedin': user_loggedin,
        
        }
    return render(request, 'rental_property/manageranalytics.html',context)
...
@login_required
@user_passes_test(lambda user: user.is_manager==True, login_url='profile')
def managed_buildings(request):
    manager1=Managers.objects.get(associated_account=request.user)
    buildings= Building.objects.filter(manager=manager1)
    context={
        'buildings':buildings,
    }
    return render(request, 'rental_property/building_list.html', context)
